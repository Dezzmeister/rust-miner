...
/**
 * Start hashing a full block. Hashes the first 5/7 chunks and copies the 6th chunk
 * to the schedule. The 6th and 7th chunks will be hashed by the GPU.
 */
pub fn start_hash(full_block: &FullBlock) -> (MessageSchedule, [u32; 8]) {
    let input: Vec<u8> = bincode::serialize(full_block).expect("Failed to serialize block");
    let (block_vec, num_chunks) = make_message_block(input.as_slice());
    let block = block_vec.as_slice();

    // If the size of a block changes, we should know. A block containing transactions
    // should be split into 7 chunks
    ...
    for i in 0..5 {
        ...
        copy_chunk_to_schedule(block, i, &mut schedule);
